...
int main( int argc, char **argv )
{
  ...
  {
    number = 917202713;
    index = searchItem( memsys, array, cmp, &number );
    printf( "%d %d", index==4242, (memsys->memctr.get>10 && memsys->memctr.get<20) );
  }
  else if (strcmp(argv[1],"present2")==0)
  { 
    number = 63542;
    index = searchItem( memsys, array, cmp, &number );
    printf( "%d %d", index==0, (memsys->memctr.get>10 && memsys->memctr.get<20) );
  }
  ...
  else
  {
    number = 917202714;
    index = searchItem( memsys, array, cmp, &number );
    printf( "%d %d", index==-1, (memsys->memctr.get>10 && memsys->memctr.get< 20 ) );
  }
